...
PHP_METHOD(elasticsearch_client, add) {
	...
	// format 'request_url' and 'request tbody'
...
/* {{{ es_client_parse - ElasticSearchClient helper */
PHPAPI void es_client_parse(INTERNAL_FUNCTION_PARAMETERS, int type, zend_string **request_url, zval **zv_body) 
{
	zval *params = NULL;
    zval *host;
	zval *port;
	...
	if(type == ES_TYPE_ADD || type == ES_TYPE_UPDATE || type == ES_TYPE_SEARCH) 
	{
...
/* {{{ es_client_request - ElasticSearchClient helper */
